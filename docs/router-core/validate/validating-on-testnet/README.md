...
Learn how to run a validator node on the Router testnet
<!-- ## Synced Node
...
```bash
routerd query slashing signing-info [validator-pubkey] --chain-id osmo-test-4
```

Example:

```bash
routerd query slashing signing-info '{"@type":"/cosmos.crypto.ed25519.PubKey","key":"HlixoxNZBPq4pBOYEimtSq9Ak4peBISVsIbI5ZHrEAU="}' --chain-id osmo-test-4
``` -->
