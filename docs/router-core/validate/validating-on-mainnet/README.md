...
Learn how to run a validator node on the Router mainnet
<!-- ## Synced Node
...
```bash
routerd tendermint show-validator
```
...
```bash
routerd query staking validators --limit 300 -o json | jq -r '.validators[] |
...
```bash
routerd query staking validators -o --limit 300 json | jq -r '.validators[] |
[.operator_address, .status, (.tokens|tonumber / pow(10; 6)),
...
```bash
routerd query slashing signing-info [validator-pubkey]
```

Example:

```bash
routerd query slashing signing-info '{"@type":"/cosmos.crypto.ed25519.PubKey","key":"HlixoxNZBPq4pBOYEimtSq9Ak4peBISVsIbI5ZHrEAU="}'
``` -->
