---
title: Interact with Router CLI
sidebar_position: 2
---

## Using the CLI

Once routerd is [installed](../router-core/routerd) and configured with the correct network, you can now send commands with it. In a new terminal window, start by running the following query command:
...
#### This is the output from `routerd status`
...
```
In this example when we install routerd as a client with the [installer](../router-core/routerd), it connects to the `http://osmosis.artifact-staking.io:26657`. 
...
```bash
routerd config node https://rpc-test.osmosis.zone:443
routerd config chain-id osmo-test-4
```

To add a  new account on your local keyring
```bash
routerd keys add testaccount --keyring-backend test

# Put the generated address in a variable for later use.
MYACCOUNT=$(routerd keys show testaccount -a --keyring-backend test)
```
...
routerd query bank balances $MYACCOUNT
```
![](../assets/query-balance.png)  

For more information about querying routerd via the CLI visit the [Cosmos documentation](https://hub.cosmos.network/main/hub-tutorials/gaiad.html).
