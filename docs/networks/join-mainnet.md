...
Make sure you have [installed the Osmosis Binary CLI](../router-core/routerd#minimum-requirements) prior to following the below instructions.
...
```bash
routerd init NODE_NAME
```

Download and place the genesis file in the osmosis config folder:

```
wget -O ~/.routerd/config/genesis.json https://github.com/router-protocol/networks/raw/main/osmosis-1/genesis.json
```
...
If you want routerd to upgrade automatically from V9 to V10, do the following steps prior to the upgrade height (4713065):

```{.sh}
mkdir -p ~/.routerd/cosmovisor/upgrades/v9/bin
cd $HOME/osmosis
git pull
git checkout v10.0.0
make build
cp build/routerd ~/.routerd/cosmovisor/upgrades/v9/bin
``` -->