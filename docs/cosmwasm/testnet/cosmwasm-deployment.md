...
:::tip
Please note this a detailed guide on how to deploy via `routerd`, it also covers additional tooling and useful tips.  You can also deploy to testnet with [Beaker](./cosmwasm-beaker.md) with a couple of commands. 
:::
...
You can easily set up an Osmosis Testnet environment using the [Osmosis Installer](https://github.com/router-protocol/osmosis-installer). 
...
```
Now you have successfully completed setting up an Router CLIent node in Testnet. In order to use `routerd` from the cli, either reload your terminal or refresh your profile with : `‘source ~/.profile’`
...
When you run the commands above, `routerd` will prompt you all the information related to that wallet in YAML (.yml) format.
...
```bash
routerd query bank balances $(routerd keys show -a wallet)
```

- `routerd query bank balances [address]` commands query the total balance of an account.
- `routerd keys show -a wallet` commands returns the address of the wallet that you created.
...
For this example, we will use the [**cw-template**](https://github.com/router-protocol/cw-tpl-osmosis) repo with counter example.

```bash
cargo generate --git https://github.com/router-protocol/cw-tpl-osmosis.git --name my-first-contract
...
RES=$(routerd tx wasm store artifacts/my_first_contract.wasm --from wallet --gas-prices 0.1uosmo --gas auto --gas-adjustment 1.3 -y --output json -b block)
```

- `routerd tx wasm store` : upload a wasm binary
...
# instantiate the contract
routerd tx wasm instantiate $CODE_ID "$INIT" \
    --from wallet --label "my first contract" --gas-prices 0.025uosmo --gas auto --gas-adjustment 1.3 -b block -y --no-admin
```

- `routerd tx wasm instantiate` : instantiate a wasm contract using CODE_ID of the uploaded binary.
...
- `routerd query wasm list-contract-by-code` : list wasm all bytecode on the chain for given code id
...
```bash
QUERY='{"get_count":{}}'
routerd query wasm contract-state smart $CONTRACT_ADDR "$QUERY" --output json
```

- `routerd query wasm contract-state smart` : calls contract with given address with query data and prints the returned result
...
routerd tx wasm execute $CONTRACT_ADDR "$TRY_INCREMENT" --from wallet --gas-prices 0.025uosmo --gas auto --gas-adjustment 1.3 -y
```

- `routerd tx wasm execute` : execute a command on a wasm contract
...
```bash
RESET='{"reset": {"count": 0}}'
routerd tx wasm execute $CONTRACT_ADDR "$RESET" --from wallet --gas-prices 0.025uosmo --gas auto --gas-adjustment 1.3 -y
```
