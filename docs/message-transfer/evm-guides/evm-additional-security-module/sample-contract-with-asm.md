...
To integrate custom ASM into their application through a simple cross-chain Ping-Pong Dapp, the user can follow the steps mentioned below.

* For information, Ping-Pong is an app where a message can be sent from the source chain to a destination chain and receive back an acknowledgment from the destination chain. So, a ping will be sent to the destination chain and a pong will be received back to the source chain.
* After deploying the application contract (Ping-pong) and the ASM contract (a sample is provided in the previous section) on respective chains, the user needs to provide the address for the ASM contract deployed on the destination chain in the bytes format while initiating a cross-chain request from the source chain in the request metadata. In the example below, the same is being done in the `iPing` function.

The code snippet for the Ping-Pong is given below:
